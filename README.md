...
├── model/          — Domain objects (Record, PersonalInfo, WorkInfo, Preferences, Compensation, History)
...
Uses the same nested structure: `personalInfo`, `workInfo`, `preferences`, `compensation`, and `history`.

- `personalInfo` and `workInfo` are **required**
- `preferences`, `compensation`, and `history` are optional
...
| `compensation.baseSalary` | Required, min 0 |
| `compensation.payFrequency` | Required, must match a lookup value (annual, monthly, bi-weekly, weekly) |
| `compensation.bonusTarget` | Optional, range 0–100 (percentage) |
| `compensation.stockOptions` | Optional, min 0 |
| `compensation.effectiveDate` | Required, ISO date (YYYY-MM-DD) |
| `compensation.overtimeEligible` | Optional, defaults to false |
...
```json
{
  ...
  "payFrequencies": [{ "value": "annual", "label": "Annual" }],
...
- **compensation** — one-to-one, cascade deletes with records
- **emergency_contacts** — one-to-many, cascade deletes with records
- **certifications** — one-to-many, cascade deletes with records
- **Lookup tables** — departments, statuses, employment_types, notification_channels, access_levels, pay_frequencies, relationships
...
| V10 | Create compensation table and pay_frequencies lookup |
| V11 | Seed compensation data for all 60 records |
